/**
 * =========================================
 * FILE INFO
 * =========================================
 * File Name   : AttendeesPage.tsx
 * Type        : Page
 * Feature     : Feature 1
 * Route       : /organizer/events/:slug/attendees
 * Source Path : src/pages/organizer/AttendeesPage.tsx
 * Used In     : Organizer Event Management
 * UI Section  : Attendees Page
 * Status      : ACTIVE
 * =========================================
 */

import { Link, useParams } from "react-router-dom";

const dummyAttendees = [
  {
    id: "att-001",
    name: "Budi Santoso",
    email: "budi@example.com",
    attendanceStatus: "REGISTERED",
  },
  {
    id: "att-002",
    name: "Siti Rahma",
    email: "siti@example.com",
    attendanceStatus: "ATTENDED",
  },
  {
    id: "att-003",
    name: "Raka Pratama",
    email: "raka@example.com",
    attendanceStatus: "REGISTERED",
  },
];

export default function AttendeesPage() {
  const { slug } = useParams();

  return (
    <main className="min-h-[calc(100vh-104px)] bg-slate-50 px-4 py-10">
      <div className="mx-auto max-w-6xl space-y-8">
        <div className="rounded-3xl border border-zinc-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-3xl font-bold text-zinc-900">Attendees Page</h1>

          <p className="mt-4 text-sm font-semibold text-zinc-600">
            Feature 1
          </p>

          <p className="mt-2 text-sm text-zinc-500">
            src/pages/organizer/AttendeesPage.tsx
          </p>

          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Link
              to={`/organizer/events/${slug}`}
              className="inline-flex items-center justify-center rounded-xl border border-zinc-300 px-5 py-3 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-100"
            >
              Back to Organizer Detail
            </Link>
          </div>
        </div>

        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
          <div className="mb-6">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-emerald-600">
              Organizer Attendees
            </p>

            <h2 className="mt-3 text-2xl font-bold text-slate-900">
              Event Attendees
            </h2>

            <p className="mt-2 text-sm leading-7 text-slate-600">
              Ini masih dummy UI untuk preview peserta event sebelum attendance
              backend disambungkan.
            </p>
          </div>

          <div className="space-y-4">
            {dummyAttendees.map((attendee) => (
              <div
                key={attendee.id}
                className="rounded-2xl border border-slate-200 bg-slate-50 p-5"
              >
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                  <div>
                    <h3 className="text-lg font-bold text-slate-900">
                      {attendee.name}
                    </h3>
                    <p className="mt-1 text-sm text-slate-600">
                      {attendee.email}
                    </p>
                    <p className="mt-1 text-sm text-slate-500">
                      Attendee ID: {attendee.id}
                    </p>
                  </div>

                  <span className="inline-flex rounded-full bg-emerald-600 px-3 py-1 text-xs font-semibold text-white">
                    {attendee.attendanceStatus}
                  </span>
                </div>
              </div>
            ))}
          </div>
        </section>
      </div>
    </main>
  );
}